...
def _extract_doc_id(item: dict) -> str | None:
    """
    Make evaluation robust to different key names from bm25_search().
    """
    for key in ("doc_id", "id", "docid", "_id"):
...
def main():
    ...
    for r in rows:
        ...
        relevant_raw = r.get("relevant_doc_ids", [])
        relevant = set(str(x).strip() for x in relevant_raw if str(x).strip())

        rel_map = r.get("relevance", {}) or {}
        rel_map = {str(k).strip(): float(v) for k, v in rel_map.items()}

        stem_mode = str(r.get("stemming_mode", settings.stemming_mode)).lower().strip()
        ...
        ranked_doc_ids = []
        for it in res:
            did = _extract_doc_id(it)
            if did:
                ranked_doc_ids.append(did)

        # DEBUG: kalau overlap 0, print contoh biar ketahuan masalahnya
        inter = relevant.intersection(set(ranked_doc_ids[:50]))
        if relevant and len(inter) == 0 and debug_shown < 3:
            debug_shown += 1
            print("\n[DEBUG] NO OVERLAP FOUND")
            print("Query:", q)
            print("Tokens:", q_tokens[:25], "...", f"(count={len(q_tokens)})")
            print("Relevant IDs:", list(relevant)[:10])
            print("Top10 Pred IDs:", ranked_doc_ids[:10])
            # show available keys of first result
            if res:
    ...
    if n == 0:
        raise RuntimeError("Tidak ada query valid di eval/queries.jsonl")
